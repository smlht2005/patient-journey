 ...
 * 
 * 對接 FHIR Observation 資源，依 LOINC code 判斷數值是否超標，
 * 產生對應 CDS Card（event-driven，即時觸發）。
 */
...
export function extractLatestValues(observations: any[]): LatestObs[] {
  const map = new Map<string, LatestObs>();
  for (const r of observations) {
    ...
    const existing = map.get(loinc);
    if (!existing || time > (existing as any)._time) {
      map.set(loinc, { loinc, value, unit, ...(({ _time: time } as any)) });
    }
  }
...
export function buildThresholdCards(observations: any[]): CdsCard[] {
  ...
  for (const rule of THRESHOLD_RULES) {
    ...
    let indicator: 'critical' | 'warning' | 'info' | null = null;
    let label = '';

    if (rule.criticalAbove && value > rule.criticalAbove) { indicator = 'critical'; label = `危急高值 ${value} ${rule.unit}`; }
    else if (rule.warnAbove && value > rule.warnAbove)    { indicator = 'warning';  label = `偏高 ${value} ${rule.unit}`; }
    else if (rule.criticalBelow && value < rule.criticalBelow) { indicator = 'critical'; label = `危急低值 ${value} ${rule.unit}`; }
    else if (rule.warnBelow && value < rule.warnBelow)    { indicator = 'warning';  label = `偏低 ${value} ${rule.unit}`; }
