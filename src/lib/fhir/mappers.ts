 ...
 * 修正：
 * - 移除未使用的 hasCoding() 死碼
 * - mapVitals 改用絕對時間 HH:mm label（避免 SSR/CSR 時間不同步）
 * - mapRadar 移除宣告但未使用的 creat 變數
 */
import {
...
function observationStatus(loinc: string, value: number): 'normal' | 'warning' | 'critical' {
  ...
  if (!t) return 'normal';
  return (t.dir === 'gt' ? value > t.warn : value < t.warn) ? 'warning' : 'normal';
}
...
export function mapObservations(resources: any[]): ObservationVM[] {
  return resources
    ...
    .map((r: any) => {
      ...
      return {
        code: loinc,
        display: r.code?.text ?? r.code?.coding?.[0]?.display ?? loinc,
        value, unit,
...
/**
 * FHIR Observation[] → VitalSignVM[]
 * 修正：使用絕對時間 "MM/DD HH:mm" 作為 label，
 * 避免 SSR 與 CSR 使用不同 Date.now() 導致圖表 x 軸跳動。
 */
export function mapVitals(resources: any[]): VitalSignVM[] {
  ...
  resources
    ...
    .forEach((r: any) => {
      ...
      // 以 4h 為單位分桶，label 用絕對時間（穩定，不依賴 Date.now()）
      const bucketMs  = Math.floor(dt.getTime() / (4 * 3600 * 1000)) * (4 * 3600 * 1000);
      const bucketDt  = new Date(bucketMs);
      const label     = `${String(bucketDt.getMonth() + 1).padStart(2, '0')}/${String(bucketDt.getDate()).padStart(2, '0')} ${String(bucketDt.getHours()).padStart(2, '0')}:00`;
      if (!buckets[label]) buckets[label] = { t: label };
      (buckets[label] as any)[key] = getObsValue(r).value;
    });

  return Object.values(buckets).sort((a, b) => (a.t ?? '').localeCompare(b.t ?? '')) as VitalSignVM[];
}

export function mapMedications(resources: any[]): MedicationVM[] {
  const labMap: Record<string, string> = {
    metformin: 'HbA1c / Glucose AC', januvia: 'HbA1c / Glucose AC',
    sitagliptin: 'HbA1c / Glucose AC', insulin: 'Glucose AC',
    lisinopril: 'Systolic BP / K+', atorvastatin: 'LDL Cholesterol',
    rosuvastatin: 'LDL Cholesterol', warfarin: 'PT/INR', aspirin: 'PT/INR',
  };
  return resources
    .filter((r: any) => r.resourceType === 'MedicationRequest' && r.status === 'active')
    .map((r: any) => {
      const name    = r.medicationCodeableConcept?.text ?? r.medicationCodeableConcept?.coding?.[0]?.display ?? '(未知藥品)';
      ...
      return {
        ...
        freq:  dosage?.timing?.code?.text ?? (dosage?.timing?.repeat?.frequency ? `${dosage.timing.repeat.frequency}x/day` : 'QD'),
  ...
  if (meds.length > 0) {
    ...
    events.push({
      ...
      detail: meds.map((r: any) => {
        const n = r.medicationCodeableConcept?.text ?? '(藥品)';
        const d = r.dosageInstruction?.[0]?.doseAndRate?.[0]?.doseQuantity;
        return `${n} — ${d ? `${d.value} ${d.unit}` : '—'}`;
      }),
      flag: 'info',
    });
  }
...
/**
 * mapRadar — 修正：移除宣告但未使用的 creat 變數
 */
export function mapRadar(observations: ObservationVM[]): RadarVM[] {
  ...
  const renalScore = Math.max(0, Math.min(100, Math.round((2.0 - creat) / 0.02)));
  return [
    { dim: '血壓',   v: Math.min(100, Math.round((sbp - 90) / 0.8)) },
    { dim: '心率',   v: Math.min(100, Math.round(pulse * 0.9)) },
    { dim: '肝功能', v: renalScore },
